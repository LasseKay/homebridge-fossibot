const { getReadModbus, getWriteModbus } = require('./functions');
...
const _REGISTERS: Record<number, RegisterDefinition> = {};
...
function getWriteModusValue(register: number, value: number) {
    const reg = _REGISTERS[register];
    switch (reg.unit) {
        case "permille":
            value = parseInt(value as any);
            if (value < 0 || value > 1000 || !((value % 10) === 0 || (value % 10) === 5)) {
                console.log(`Register [${reg.name}] does not support value ${value}`);
                return;
            }
            break;
        case "int":
            ...
            if (reg.possibleValues) {
                if (!reg.possibleValues.includes(value)) {
                    console.log(`Register [${reg.name}] does not support value ${value}`);
                    return;
                }
            } else {
    ...
    console.log(`Register [${reg.name}] will switch to ${value}`);
...
function REGDischargeLowerLimit(value: number) { return getWriteModusValue(REGISTER_DISCHARGE_LIMIT, value); }
function REGChargeUpperLimit(value: number) { return getWriteModusValue(REGISTER_CHARGING_LIMIT, value); }
function REGStopChargeAfter(value: number) { return getWriteModusValue(REGISTER_STOP_CHARGE_AFTER, value); }
function REGMaxChargeCurrent(value: number) { return getWriteModusValue(REGISTER_MAXIMUM_CHARGING_CURRENT, value); }
function REGScreenRestTime(value: number) { return getWriteModusValue(REGISTER_SCREEN_REST_TIME, value); }
function REGAcStandbyTime(value: number) { return getWriteModusValue(REGISTER_AC_STANDBY_TIME, value); }
function REGDcStandbyTime(value: number) { return getWriteModusValue(REGISTER_DC_STANDBY_TIME, value); }
function REGUsbStandbyTime(value: number) { return getWriteModusValue(REGISTER_USB_STANDBY_TIME, value); }
function REGSleepTime(value: number) { return getWriteModusValue(REGISTER_SLEEP_TIME, value); }
